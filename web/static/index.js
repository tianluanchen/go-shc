...
getFormElemHandler("#language").setOptions(
    languageOptions, (languageOptions.find((e) => {
        const v = e[1];
...
const generateObfuscateCodes = async () => {
    ...
    const [minVarNameLength, sliceLength] = obfuscateLevelSelect.getValue().split(",").map(e => parseInt(e));
...
document.querySelectorAll("textarea").forEach(e => {
    e.addEventListener("input", () => {
        ...
        elem.textContent = (elem.dataset.tmpl || "").replace("#char#", count + '').replace("#line#", lines + '').replace("#size#", formatSize(size));
    })
})
...
function copyWithBtn(s, btn) {
    ...
    asyncWrapperWithElem(() => copyText(s).finally(() => {
